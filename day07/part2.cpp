    ...
    uint32_t index)
{
    ...
    {
        int64_t result = (index / (int64_t)pow(3, (size - (operation_index + 1)))) % 3;
        switch (result)
        {
            case 0:
            {
            ...
            case 2:
            {
                operations.push_back(CONCATINATE);
                break;
            }
        }
    }
    ...
    std::vector<int64_t> numbers)
{
    ...
    std::vector<Operation> operations;
    for(int operation_combo = 0; operation_combo < pow(3, numbers.size() - 1); operation_combo++)
    {
        ...
        switch(operations[0])
        {
            ...
            case CONCATINATE:
            {
                top_operation = new Concatination(new Constant(numbers[0]),
                                                  new Constant(numbers[1]));
                break;
            }
        }
        for (int operation_index = 1; operation_index < operations.size(); operation_index++)
        {
            switch(operations[operation_index])
            {
                ...
                case CONCATINATE:
                {
                    top_operation = new Concatination(top_operation,
                                                      new Constant(numbers[operation_index + 1]));
                    break;
                }
            }
